...
file_path = "reg-capstat-2024-Fall-semester-11-14-2024.xlsx"
...
def process_course_load(data: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = df.groupby(['Instructor', 'Course Title']).agg({
        'Section_Only': lambda x: ' & '.join(sorted(x)),
        'Title': 'first',
    ...
    grouped = pd.merge(grouped, total_enrollment, on='Instructor')
    grouped = pd.merge(grouped, total_minimum_units, on='Instructor')

    grouped = grouped.sort_values(['Instructor', 'Course Title'])

    output_rows = []
    prev_instructor = None
    for instructor, group in grouped.groupby('Instructor'):
        ...
        total_row = {
            ...
            'Sections': '',
            'Title': '',
            'Enrollment Count': group['Total Enrollment'].iloc[0], 
            'Department Name': '',
            'Stream': '',
            'Load Status': 'N/A', 
            'Payment Initiator': 'N/A',  
            'Minimum Units': group['Total Minimum Units'].iloc[0] 
        }
...
save_to_existing_excel_with_dropdown_and_color(processed_df, 'combined_courses_file_d.xlsx', 'Course Load')

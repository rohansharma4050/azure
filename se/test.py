...
result['Instructor(s)/Teaching Assistant'] = result['Instructor(s)/Teaching Assistant'].fillna('TBA')

result.loc[result['Instructor(s)/Teaching Assistant'] == 'TBA', 'Department Name'] = 'SE'
...
result['Instructor'] = result['Instructor'].str.strip()
...
def process_course_load(data: pd.DataFrame, default_load_status='On Load') -> pd.DataFrame:
    # Keep track of original courses for validation
    df = data.copy()

    df['Course Title'] = df['Course Title'].str.strip().str.upper()
    df['Instructor'] = df['Instructor'].str.strip().str.upper()
    original_courses = set(df['Course'].unique())
    ...
    # Create course sections grouping
    ...
    def should_combine_sections(sections):
        ...
        return len(section_set) == 2 and 'A' in section_set and 'A-U' in section_set
    ...
    for instructor, instructor_group in grouped.groupby('Instructor'):
        # Add separator row between instructors
        ...
        # Process each course for this instructor
        course_groups = instructor_group.groupby('Course Title')
        for course_title, course_group in course_groups:
            course_key = (instructor, course_title)
    

            
            if combine_map.get(course_key, False):
                # Combine A and A-U sections
                combined_row = {
                    'Instructor': '',
                    'Course Title': course_title,
                    'Sections': 'A & A-U',
                    'Title': course_group['Title'].iloc[0],
                    'Enrollment Count': course_group['Enrollment Count'].sum(),
                    'Department Name': course_group['Department Name'].iloc[0],
                    'Stream': course_group['Stream'].iloc[0],
                    'Load Status': default_load_status,
                    'Payment Initiator': 'N/A',
                    'Minimum Units': course_group['Minimum Units'].sum(),
                }
                output_rows.append(combined_row)
            else:
                # Add individual rows for each section
    ...
    # Validate no courses were missed
    processed_courses = set(result[result['Course Title'] != '']['Course Title'].unique())
    missing_courses = {course for course in original_courses if clean_course_number(course) not in processed_courses}
    if missing_courses:
        print("Warning: The following courses might have been missed:")

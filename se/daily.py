...
def process_course_load(data: pd.DataFrame, default_load_status='On Load') -> pd.DataFrame:
    ...
    df['Section_Only'] = df['Course'].apply(extract_section).str.strip().str.upper()
    ...
    def should_combine_sections(sections):
        ...
        return 'A' in section_set and 'A-U' in section_set
    ...
    for instructor, instructor_group in grouped.groupby('Instructor'):
        ...
        total_enrollment = instructor_group['Enrollment Count'].sum()
        total_minimum_units = instructor_group['Minimum Units'].sum()
        ...
        total_row = {
            ...
            'Sections': '',
            'Title': '',
            'Enrollment Count': total_enrollment,
            'Department Name': '',
            'Stream': '',
            'Load Status': default_load_status,
            'Payment Initiator': 'N/A',
            'Minimum Units': total_minimum_units
        }
        ...
        for course_title, course_group in course_groups:
            ...
            if combine_map.get(course_key, False):
                ...
                combined_row = {
                    ...
                    'Enrollment Count': course_group[course_group['Section_Only'].isin(['A', 'A-U'])]['Enrollment Count'].sum(),
                    'Department Name': course_group['Department Name'].iloc[0],
                    'Stream': course_group['Stream'].iloc[0],
                    'Load Status': default_load_status,
                    'Payment Initiator': 'N/A',
                    'Minimum Units': course_group[course_group['Section_Only'].isin(['A', 'A-U'])]['Minimum Units'].sum(),
                }
                output_rows.append(combined_row)

                remaining_sections = course_group[~course_group['Section_Only'].isin(['A', 'A-U'])]
                for _, row in remaining_sections.iterrows():
                    course_row = {
                        'Instructor': '',
